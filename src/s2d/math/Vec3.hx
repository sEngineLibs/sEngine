...
@:nullSafety
#if !macro @:build(s2d.math.SMath.Swizzle.generateFields(3)) #end
abstract Vec3(FastVector3) to FastVector3 from FastVector3 {
	#if !macro
	...
	public inline function log2():Vec3 @:privateAccess {
		return new Vec3(SMath.log2f(x), SMath.log2f(y), SMath.log2f(z));
	}
